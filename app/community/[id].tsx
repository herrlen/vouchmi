...
import { View, Text, FlatList, TextInput, Pressable, StyleSheet, KeyboardAvoidingView, Platform, Alert, Linking, Share } from "react-native";
import { useLocalSearchParams, useNavigation } from "expo-router";
import { useAuth, useApp } from "../../src/lib/store";
import PostCard from "../../src/components/PostCard";
import LinkEmbed from "../../src/components/LinkEmbed";
import { colors } from "../../src/constants/theme";
import { communities as communitiesApi } from "../../src/lib/api";
...
export default function CommunityDetail() {
  ...
  const inviteFriends = async () => {
    ...
    try {
      ...
      await Share.share({
        message: `Hey! Ich hab dich in meine TrusCart-Community eingeladen. Tritt bei über diesen Link:\n${invite_link}`,
      });
    } catch (e: any) {
      Alert.alert("Fehler", e.message);
    }
  ...
  return (
    <KeyboardAvoidingView style={s.container} behavior={Platform.OS === "ios" ? "padding" : undefined} keyboardVerticalOffset={90}>
      ...
      <View style={s.tabs}>
        ...
        <Pressable style={s.tab} onPress={inviteFriends} hitSlop={6}>
          <Text style={s.tabText}>✉︎</Text>
        </Pressable>
      </View>
      ...
      {tab === "feed" && (
        <FlatList
          ...
          refreshing={false}
          renderItem={({ item }) => <PostCard post={item} onLike={likePost} onHide={(pid) => id && loadFeed(id)} />}
