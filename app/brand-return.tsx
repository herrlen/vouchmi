...
export default function BrandReturn() {
  ...
  useEffect(() => {
    ...
    const poll = async (): Promise<void> => {
      ...
      try {
        ...
        if (status.has_active && status.plan_type === "brand") {
          // Rolle neu laden (role: user -> brand nach Webhook-Activation).
          await initAuth();
        ...
        // ignorieren — polling versucht weiter
      }
      if (++attempts >= 10) {
        setState("pending");
        return;
      ...
      <View style={s.centered}>
        {state === "polling" && (
          <>
            ...
            <Text style={s.body}>Wir aktivieren dein Brand-Abo. Das kann ein paar Sekunden dauern.</Text>
          </>
        )}
        {state === "success" && (
          <>
            <CheckCircle2 color="#25D366" size={64} strokeWidth={1.8} />
            <Text style={s.title}>Du bist jetzt Brand!</Text>
            <Text style={s.body}>Alle Brand-Features sind freigeschaltet.</Text>
          </>
        )}
        {state === "pending" && (
          <>
        ...
        {state === "cancelled" && (
          <>
            ...
            <Pressable style={s.btn} onPress={() => router.replace("/brand")}>
