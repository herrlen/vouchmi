...
<head>
    ...
    <title>E-Mail bestätigen — Vouchmi</title>
    <style>
...
<body>
    <div class="card">
        ...
        @if (empty($token) || empty($email))
            ...
            <p class="error">Dieser Bestätigungs-Link ist nicht vollständig. Bitte fordere eine neue Verifizierungs-E-Mail in der App an.</p>
        @else
            <h1>E-Mail bestätigen</h1>
            <p>Tippe auf den Button, um die Bestätigung in der Vouchmi-App abzuschließen.</p>

            <a id="open-app" class="btn" href="vouchmi://verify-email?token={{ urlencode($token) }}&email={{ urlencode($email) }}">
    ...
    <script>
        ...
        (function () {
            var deepLink = 'vouchmi://verify-email?token={{ urlencode($token) }}&email={{ urlencode($email) }}';
